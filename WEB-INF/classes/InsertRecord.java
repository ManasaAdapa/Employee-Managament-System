...
public void doPost(HttpServletRequest request, HttpServletResponse response)	throws ServletException, IOException {
	...
	try{
		...
		if(!res.equals("error")){
			RequestDispatcher rd=request.getRequestDispatcher("InsertRecord.jsp?t1=Employee details added with ID = "+res);
			rd.forward(request, response);
		}else{
			RequestDispatcher rd=request.getRequestDispatcher("InsertRecord.jsp?t1="+res);
			rd.forward(request, response);
		}
	}catch(Exception e){
		e.printStackTrace();
