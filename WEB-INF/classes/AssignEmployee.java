...
public void doPost(HttpServletRequest request, HttpServletResponse response)	throws ServletException, IOException {
	...
	try{
		...
		if(!res.equals("error")){
			RequestDispatcher rd=request.getRequestDispatcher("AssignEmployee.jsp?t1=Employee assigned to "+assignto);
			rd.forward(request, response);
		}else{
			RequestDispatcher rd=request.getRequestDispatcher("AssignEmployee.jsp?t1="+res);
			rd.forward(request, response);
		}
	}catch(Exception e){
		e.printStackTrace();
